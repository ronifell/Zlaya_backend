...
Hierarquia: alimentação/saciedade + pós-mamada/postural (arroto, vertical) → perguntar o que falta → só depois janela/estímulos.
PROIBIDO: chamar de soneca curta; hipótese de estímulos/janela perdida sem dados; "sequência noturna"; "sinais de saciedade no RN".`,
    criteria: [
      ...
      { id: 'feeding_priority', w: 20, pass: (t) => /alimenta|saciedad|produ[cç][aã]o|transfer[eê]ncia|mamada/i.test(t) },
      { id: 'postural_ask', w: 20, pass: (t) =>
      ...
      { id: 'no_night_sequence', w: 15, pass: (t) => !/sequ[eê]ncia noturna/i.test(t) },
      { id: 'no_rn_satiety_block', w: 10, pass: (t) => !/sinais de saciedade no RN/i.test(t) },
  ...
  },
  {
    ...
    dossierSummary: `Dado central: vigília excessiva (~1h40–2h) porque inicia após 1h–1h15 e ainda demora 40–45 min para dormir. Referência 45min–1h.
PROIBIDO inventar mamada noturna insuficiente / baixa produção à noite sem relato noturno.
Vertical geral 20–30 (não 30–40 sempre). Humano sugeriu fracionar soneca manhã ~1h30–2h e caprichar mamadas.`,
    criteria: [
      { id: 'excess_wake_central', w: 25, pass: (t) => /(1h\s*40|1h40|2h|demais|excessiv|muito tempo|longo).{0,40}(acord|vig[ií]lia)|vig[ií]lia.{0,40}(excess|longo|demais)|45\s*min.{0,30}1\s*h/i.test(t) },
      { id: 'wake_ref_45_60', w: 20, pass: (t) => /45\s*min/i.test(t) && /1\s*h|1\s*hora/i.test(t) },
      { id: 'no_invented_night_supply', w: 25, pass: (t) => !/mamada noturna insuficiente|produ[cç][aã]o de leite durante a noite|baixa produ[cç][aã]o.{0,20}noite/i.test(t) },
      { id: 'vertical_20_30', w: 15, pass: (t) => /20\s*a\s*30/i.test(t) || !/30\s*a\s*40\s*minutos ap[oó]s/i.test(t) },
      { id: 'day_naps_focus', w: 15, pass: (t) => /soneca|tarde|manh[aã]|fracion/i.test(t) },
    ],
  },
  {
    id: '40d-bottle',
    ...
    dossierSummary: `~20 min OK se retirada efetiva. NÃO chamar sucção pós-20min de hábito sem saciedade.
Mamadeira: 90–120 ml (não 60–90). Indicar só conteúdo de mamadeira/alimentação, não maus hábitos / sono noturno.`,
    criteria: [
      { id: 'breast_20min', w: 20, pass: (t) => /20\s*minutos/i.test(t) },
      { id: 'no_habit_label', w: 20, pass: (t) => !/h[aá]bito que pode ser corrigido|mau h[aá]bito/i.test(t) },
      { id: 'volume_90_120', w: 35, pass: (t) => /90\s*(a|–|-|até)\s*120|90\s*–\s*120/i.test(t) },
      { id: 'no_60_90', w: 15, pass: (t) => !/60\s*(a|–|-)\s*90/i.test(t) },
  ...
  },
  {
    ...
    dossierSummary: `0–3 meses: NÃO classificar como mau hábito. Não indicar aula de maus hábitos.
Investigar mudança recente (5 dias). Respeitar manter chupeta. Bebê já dorme sozinho no berço — não pedir contenção no colo como eixo. Não perguntar "mama até dormir" contradizendo o relato.
Investigar: alimentação, vigília, desconforto, papel da sucção.`,
    criteria: [
      { id: 'no_mau_habito', w: 25, pass: (t, meta) => !/\b(e|eh|é)\s+(um\s+)?mau\s+h[aá]bito\b|classific\w*\s+como\s+mau\s+h[aá]bito|desenvolvendo\s+um\s+mau\s+h[aá]bito|aula sobre maus h[aá]bitos|tirando os maus h[aá]bitos/i.test(t + ' ' + (meta.lessonsText || '')) },
      { id: 'recent_change', w: 20, pass: (t) => /mudan[cç]a recente|at[eé] (cinco|5) dias|padr[aã]o (anterior|recente)|antes (fazia|dormia)/i.test(t) },
      ...
      { id: 'investigate_not_blame', w: 20, pass: (t) => /alimenta|saciedad|vig[ií]lia|desconforto|suc[cç][aã]o/i.test(t) },
      { id: 'no_contradict_solo_sleep', w: 10, pass: (t) => !/quanto tempo ele mama at[eé] dormir/i.test(t) },
      { id: 'no_colo_as_main_fix', w: 10, pass: (t) => !/conten[cç][aã]o no colo.{0,40}(interrom|h[aá]bito)/i.test(t) },
    ],
  },
  {
    id: '45d',
...
Janela 45min–1h15. Investigar última soneca. Usar gênero feminino (Lara). Sem PASSO 1 desnecessário.`,
    criteria: [
      { id: 'night_19_20', w: 25, pass: (t) => /19h.{0,15}20h|19\s*h.{0,15}20\s*h|entre 19.{0,10}20/i.test(t) },
      { id: 'not_recommend_2130_22', w: 20, pass: (t) => /21h?30|22h|22:00/i.test(t) && /n[aã]o .{0,20}recomend|n[aã]o [eé] o recomend|n[aã]o recomendado/i.test(t) },
      { id: 'bath_postpones', w: 15, pass: (t) => /banho.{0,40}(posterg|atras|adiar|mais tarde)|21h?30.{0,40}(posterg|atras)/i.test(t) },
      { id: 'wake_45_to_115', w: 15, pass: (t) => /45\s*min/i.test(t) },
      { id: 'female_gender', w: 15, pass: (t) => !/\b(ele|dele|nele)\b/i.test(t) || /\b(ela|dela|nela|Lara)\b/i.test(t) },
      { id: 'no_passo1', w: 10, pass: (t, meta) => !/PASSO 1|perceba o comportamento/i.test(t + ' ' + (meta.lessonsText || '')) },
    ],
  },
  {
    id: '49d',
    ...
    dossierSummary: `Hierarquia: como acorda → alimentação/saciedade → desconforto → vigília 45min–1h → chupeta só se ligada ao despertar.
PROIBIDO: mínimo 4–5 sonecas; vertical 30–40 para todas; culpar chupeta primeiro; vigília só 1h–1h15.`,
    criteria: [
      { id: 'how_wakes', w: 20, pass: (t) =>
        /como .{0,40}(acord|despert)|como o beb[eê] acorda|acorda da soneca|acord.{0,40}(tranquil|chor|desconfort|mam)/i.test(t)
      },
      { id: 'wake_45_60', w: 25, pass: (t) => /45\s*min/i.test(t) && /1\s*h|1\s*hora/i.test(t) },
      { id: 'no_min_naps', w: 20, pass: (t) => !/(garant|busque|imponha|m[ií]nimo de)\s*.{0,20}4 a 5 sonecas|garantir um m[ií]nimo de 4 a 5/i.test(t) || /n[aã]o h[aá] m[ií]nimo|n[aã]o imponha|n[uú]mero varia/i.test(t) },
      { id: 'vertical_20_30', w: 15, pass: (t) => /20\s*a\s*30/i.test(t) || !/30\s*a\s*40 minutos ap[oó]s (todas|as mamadas)/i.test(t) },
      { id: 'pacifier_not_first', w: 20, pass: (t) => !(/principal.{0,40}chupeta|chupeta.{0,40}(principal|caus)/i.test(t) && !/antes de|somente se|s[oó] ent[aã]o/i.test(t)) },
    ],
  },
  {
    id: '51d',
    ...
    dossierSummary: `Sem mau hábito. Hierarquia: vigília 45min–1h → alimentação/saciedade → peito fome vs adormecer → execução do travesseiro → transferência.
Responder: sem prazo fixo. Sem ~10 min de choro. Perguntas de refinamento na hierarquia correta.`,
    criteria: [
      { id: 'no_mau_habito', w: 15, pass: (t, meta) => !/mau h[aá]bito|maus h[aá]bitos/i.test(t + ' ' + (meta.lessonsText || '')) },
      { id: 'wake_first', w: 15, pass: (t) => /45\s*min|vig[ií]lia/i.test(t) },
      { id: 'feeding_before_behavior', w: 15, pass: (t) => /mamada efetiva|saciedad|alimenta|retirando leite|fome/i.test(t) },
      { id: 'no_fixed_timeline', w: 20, pass: (t) => /n[aã]o existe prazo|sem prazo|n[aã]o h[aá] prazo|prazo fixo|depende da (repeti[cç][aã]o|consist[eê]ncia)/i.test(t) },
      { id: 'no_10min_cry', w: 15, pass: (t) => !/cerca de 10 minutos|em torno de 10 minutos|10 minutos.{0,20}(choro|acalmar)/i.test(t) },
      { id: 'travesseiro_execution', w: 10, pass: (t) => /travesseiro/i.test(t) && /(como|execu[cç]|realiz|aplic)/i.test(t) },
      { id: 'hierarchy_questions', w: 10, pass: (t) => /(quanto tempo|vig[ií]lia|mamada efetiva|saciedad)/i.test(t) },
    ],
  },
];

function scoreDeterministic(answerText, lessons, c) {
  const meta = {
    lessonsText: (lessons || [])
...
async function main() {
  ...
  for (const c of CASES) {
    ...
    const det = scoreDeterministic(text, lessons, c);
